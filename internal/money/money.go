...
// that establishes its currency. A Money with a zero Currency denominates
// nothing and is rejected by arithmetic rather than silently treated as some
// default currency.
type Money struct {
	minorUnits int64
	currency   Currency
...
// New returns the amount of the given whole minor units in c.
...
// MinorUnits returns the exact minor-unit count.
...
// Equal reports whether two amounts are the same value in the same currency.
...
// differently. Addition of minor units is exact integer arithmetic, so no
// rounding occurs and no error accumulates however many amounts are summed.
...
// is the zero amount in c. The result is an exact integer sum of minor units,
// with no rounding applied at the total.
...
// Decimal returns the amount in major units as an exact decimal - 1255 minor
// units of USD becomes 12.55. The conversion is exact: it scales an integer by
// a power of ten and never passes through a floating-point representation.
func (m Money) Decimal() decimal.Decimal {
	return decimal.New(m.minorUnits, -m.currency.Exponent)
}

// String renders the amount for humans and logs, e.g. "12.55 USD".
...
// number of minor units, half-up: to the nearest whole minor unit, with an
// amount falling exactly halfway rounded away from zero.
//
// This is the single place the platform's rounding policy is expressed. Half-up
// is chosen for explicability - the rule can be stated to a customer disputing a
// line item in one sentence. The trade-off is accepted knowingly: every tie
// resolves in the biller's favour, so across high fee volume this accrues a
// small systematic surplus to the platform, where half-even would split ties.
func FromMinorUnitsDecimal(d decimal.Decimal, c Currency) Money {
	return Money{minorUnits: d.Round(0).IntPart(), currency: c}
}

// FromMajorUnitsDecimal converts a major-unit amount to Money, rounding to whole
// minor units half-up. 12.554 USD becomes 1255 minor units; 12.555 becomes 1256.
...
// half-up. This is the derivation a percentage fee goes through: the rate
// carries the precision, the product is rounded once, and the rounded value is
// both what is stored and what is charged.
...
// ParseExact parses a major-unit decimal string such as "12.55" into Money,
// rejecting any amount carrying more decimal places than c permits.
//
// Excess precision is an error rather than something to round away: rounding
// belongs to rate derivation inside the system, not to the interpretation of an
// amount a caller stated explicitly.
...
//
// The major-unit amount is carried as a JSON *string*, never a JSON number.
// A number would invite every consumer's parser to route the value through an
// IEEE-754 double on the way in, which is exactly the precision loss this
// package exists to prevent. minorUnits is included as the exact integer form so
// a consumer never has to parse the decimal at all.
...
//
// The currency is always required. The amount may be given as exact minorUnits,
// as a major-unit decimal string, or as both - and when both are given they must
// agree. A disagreement is rejected rather than resolved by precedence: it means
// the caller's two statements of the same charge differ, which is a defect worth
// surfacing loudly in a system that moves money.
